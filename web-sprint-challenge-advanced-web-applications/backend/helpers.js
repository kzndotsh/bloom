...
const articleSchema = yup.object().shape({
    ...
    topic: yup.string().required('topic is required').oneOf(topics),
});
...
async function postArticle(token, article) {
    ...
    try {
        validatedArticle = await articleSchema.validate(article, {
            stripUnknown: true,
        });
...
async function updateArticle(token, article, article_id) {
    ...
    if (!articles.find((art) => art.article_id == article_id)) {
        return [
            404,
            {
                message: `Ouch: Article with article_id ${article_id} does not seem to exist!`,
            },
        ];
    }
    try {
