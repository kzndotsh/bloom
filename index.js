...
class Car {
  ...
  drive(distance) {
    const limit = this.tank * this.milesPerGallon;
    if(distance <= limit) {
      this.odometer = this.odometer + distance;
      this.tank = this.tank - (distance / this.milesPerGallon)
    }else{
      this.odometer = this.odometer + limit;
      this.tank = 0;
      return `I ran out of fuel at ${this.odometer} miles!`;
    }
  }
}
const car = new Car('honda', 25);
car.fill(50);
car.drive(100);
console.log(car);
...
const person = new Lambdasian({name: 'Logan', age: '26', location: 'Charlotte'});
console.log(person);
person.speak();
